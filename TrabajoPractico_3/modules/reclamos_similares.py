...
# Descarga de stopwords (fuera de la función para evitar descargas repetidas)
...
def reclamos_similares(reclamos_same_depto, texto_objetivo):
    ...
    """
    # Filtrar palabras del texto objetivo y convertir a minúsculas
    palabras_filtradas_objetivo = [palabra.lower() for palabra in texto_objetivo.split() if palabra.lower() not in stopwords_es]
    ...
    for descripcion, id_reclamo in reclamos_same_depto:
        palabras_filtradas = [palabra.lower() for palabra in descripcion.split() if palabra.lower() not in stopwords_es]
        ...
        prom_objetivo = (coincidencias / len(lista_objetivo)) * 100 if lista_objetivo else 0
        prom_reclamo = (coincidencias / len(lista_reclamo)) * 100 if lista_reclamo else 0
